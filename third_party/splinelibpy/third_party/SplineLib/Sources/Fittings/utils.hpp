...
inline std::vector<double> ComputeKnotVector(int& degree,
                                             int& num_points,
                                             std::vector<double>& u_k) {

    int i, j;
    std::vector<double> knot_vector;
    knot_vector.assign(num_points + degree + 1, 0.0);
    ...
    for (i = num_points; i < num_points + degree + 1; i++) {
        knot_vector[i] = 1.0;
    }
...
inline std::vector<double> BuildCoefficientMatrix(int& degree,
    ...
    coefficient_matrix.assign(num_points * num_points, 0.0);
    std::vector<double> basis_function;
    int i, j, k, span;

    for (i = 0; i < num_points; i++) {
        span = FindSingleKnotSpan(degree, knot_vector, num_points, u_k[i]);
        basis_function = BasisFunction(degree, knot_vector, span, u_k[i]);
        k = 0;
        for (j = span - degree; j < span + 1; j++) {
            coefficient_matrix[i * num_points + j] = basis_function[k];
            k++;
        }
    }
