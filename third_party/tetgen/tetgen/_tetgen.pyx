...
cdef extern from "tetgen_wrap.h":
    cdef cppclass tetgenio_wrap:
        ...
        void LoadPoly(int, double*, int, int*, int*)
        void LoadInput(int, double*, int, int*, int*, int*, int, double*, int, double*, int, double*, int, int*, int*, int*, int*, double*, int*)
...
cdef class PyTetgenio:
    ...
    def ReturnBoundaryConditions(self):
        """ Returns boundary markers from tetgen """

        # Create python copy of array
        cdef int nfaces = self.c_tetio.numberoftrifaces
        cdef double [::1] bcs = np.empty(nfaces)

        cdef int i 
        for i in range(nfaces):
            bcs[i] = self.c_tetio.trifacemarkerlist[i]

        return np.asarray(bcs)


    def ReturnTriFaces(self):
        """ Returns triangle faces """

        # Create python copy of array
        cdef int nfaces = self.c_tetio.numberoftrifaces
        cdef double [::1] trifaces = np.empty(nfaces * 3)

        cdef int i
        for i in range(nfaces * 3):
            trifaces[i] = self.c_tetio.trifacelist[i]

        return np.asarray(trifaces).reshape((-1, 3))
    ...
    def LoadPoly(self, double [::1] points, int nfaces, int [::1] nfacepoints, int [::1] faces):
        """ Loads points and faces based on PLC to TetGen """
        npoints = points.size/3
        self.c_tetio.LoadPoly(npoints, &points[0], nfaces, &nfacepoints[0], &faces[0])
    ...
    def LoadInput(
        ...
        double [::1] hfacepoints,
        int nhfaces,
        int [::1] nhfacepolygons,
        int [::1] nhfacepolygonpoints,  # pperhface
        int [::1] hfacepolygons,
        int [::1] nhfaceholes,
        double [::1] hfaceholes,
        int [::1] hfacemarkers
        ...
        """ Loads points and faces based on PLC to TetGen """
        npoints = points.size/3
        nholes = int(holes.size/3) # incase they are zero
        nregions = int(regions.size/5) #incase they are zero
        nhfacepoints = int(hfacepoints.size/3)

        self.c_tetio.LoadInput(
            npoints,
            &points[0],
            nfaces,
            &nfacepoints[0],
...
def Tetrahedralize(
    ...
    nhfh=None, # nhfaceholes -> h per hface
    hfh=None, # hfaceholes
    hfmarkers=None,
    ...
    # Vertices per facets
    if vperf is not None:
        if not vperf.flags['C_CONTIGUOUS']:
            if vperf.dtype != ctypes.c_int:
                vperf = np.ascontiguousarray(vperf, dtype=ctypes.c_int)
            else:
                vperf = np.ascontiguousarray(vperf)
 
        elif vperf.dtype != ctypes.c_int:
            vperf = vperf.astype(ctypes.c_int)

    # Facet markers
    if fmarkers is not None:
        if not fmarkers.flags['C_CONTIGUOUS']:
            if fmarkers.dtype != ctypes.c_int:
                fmarkers = np.ascontiguousarray(fmarkers, dtype=ctypes.c_int)
            else:
                fmarkers = np.ascontiguousarray(fmarkers)
 
        elif fmarkers.dtype != ctypes.c_int:
            fmarkers = fmarkers.astype(ctypes.c_int)

    # Holes
    if holes is not None:
        holes = np.ascontiguousarray(holes, dtype=np.double)
    else:
        holes = np.ascontiguousarray(np.empty(0), dtype=np.double)

    # Regions
    if regions is not None:
        regions = np.ascontiguousarray(regions, dtype=np.double)
    else:
        regions = np.ascontiguousarray(np.empty(0), dtype=np.double)

    # Here, assume that if hfv is defined, other necessary variables are also 
    # defined.
    if hfv is not None:
        hfv = np.ascontiguousarray(hfv, dtype=np.double)
        nhf = int(nhf)
        nhfp = np.ascontiguousarray(nhfp, dtype=ctypes.c_int)
        pperhf = np.ascontiguousarray(pperhf, dtype=ctypes.c_int)
        hfp = np.ascontiguousarray(hfp, dtype=ctypes.c_int)
        nhfh = np.ascontiguousarray(nhfh, dtype=ctypes.c_int)
        hfh = np.ascontiguousarray(hfh, dtype=np.double)
        hfmarkers = np.ascontiguousarray(hfmarkers, dtype=ctypes.c_int)
    else:
        hfv = np.ascontiguousarray(np.empty(0), dtype=np.double)
        nhf = 0
        nhfp = np.ascontiguousarray(np.empty(0), dtype=ctypes.c_int)
        pperhf = np.ascontiguousarray(np.empty(0), dtype=ctypes.c_int)
        hfp = np.ascontiguousarray(np.empty(0), dtype=ctypes.c_int)
        nhfh = np.ascontiguousarray(np.empty(0), dtype=ctypes.c_int)
        hfh = np.ascontiguousarray(np.empty(0), dtype=np.double)
        hfmarkers = np.ascontiguousarray(np.empty(0), dtype=ctypes.c_int)
    ...
    if vperf is None:
        ...
        if fmarkers is None: 
            tetgenio_in.LoadPoly(v.ravel(), nf, vperf.ravel(), f.ravel())
        else:
            tetgenio_in.LoadInput(
                v.ravel(),
                nf,
                vperf.ravel(),
                f.ravel(),
    ...
    bcs = tetgenio_out.ReturnBoundaryConditions()
    tfs = tetgenio_out.ReturnTriFaces()

    return nodes, tets, bcs, tfs
